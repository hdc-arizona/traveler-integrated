...
export default () => {
    ...
    try {
      for (const rule of Array.from(styleSheet.cssRules || styleSheet.rules)) {
        if (rule.style && rule.style.filter) {
          const hexCode = /#recolorImageTo(......)/.exec(rule.style.filter);
  ...
  const recolorFilters = d3.select('#recolorImageFilters')
